...
function Hero() {
    return(
        <div className="grid-bg w-full flex flex-col items-center justify-center gap-4 text-center pt-24 pb-12 px-4">
    ...
    return(
        <div className="w-full flex flex-col items-center justify-center gap-4 py-24 px-4">
            <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-3 p-4 gap-8">
                {tiers.map((t, i) => (
                    <div key={i} className={`flex flex-col p-8 gap-8 rounded-2xl border relative shadow-lg bg-secondary/50
                    ${t.special ? 'glow' : ''} hover:translate-y-[-6px]`}>
                        {t.special ? 
                        ...
                        <div className="flex flex-col gap-2 text-sm text-foreground/60">
                            {t.specs.map((s, index) => (
                                <span key={index} className="flex items-center gap-2"><BsCheck /> {s}</span>
                            ))}
                        </div>
